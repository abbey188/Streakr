...
function TeamColumn({ name, teamName, players }: { name: string; teamName: string; players: RosterPlayer[] }) {
  ...
  const Row = (p: RosterPlayer) => (
    <div key={`${p.n}-${p.name}`} className="flex items-start gap-2 py-1">
      <span className="w-5 text-right font-mono text-[10px] text-[#A2A7AF] tabular-nums flex-shrink-0 pt-px">{p.n ?? "–"}</span>
      <span className="text-[12px] text-slate-200 leading-tight">{p.name}</span>
      {p.pos === "GK" && <span className="ml-auto text-[8.8px] font-mono font-bold text-[#A2A7AF] uppercase tracking-wider flex-shrink-0 pt-0.5">GK</span>}
    </div>
  );
  return (
    <div className="min-w-0 flex-1">
      <div className="flex items-center gap-2 mb-2 pb-2 border-b border-white/5">
        ...
        <span className="text-[12px] font-black uppercase tracking-tight text-[#FF4E00] leading-tight">{name}</span>
      </div>
      <div>{xi.map(Row)}</div>
      {bench.length > 0 && (
        <>
          <div className="text-[8.8px] font-mono font-bold uppercase tracking-widest text-[#A2A7AF] mt-3 mb-1">Subs</div>
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} aria-label="Close" className="text-[#A2A7AF] hover:text-white transition"><X className="w-4 h-4" /></button>
        </div>
